...
public class DataManager
{
    ...
    private UserExpTableSO _userExpTable;
    ...
    public UserExpTableSO UserExpTable { get { return _userExpTable; } }

    // 유저 정보와 경험치 표를 불러오며, 유저 정보가 없을 경우 생성
    public async Task Load()
    {
        _userExpTable = await AddressableHelper.LoadingToPath<UserExpTableSO>(UserExpTablePath);
    ...
    public async void Save()
    {
        ...
        if(user == null)
        {
            ...
            foreach(StageSO so in Managers.Main.stageDatas.StageList)
            {
